...
impl SearchSpace<FBNode, Vec<JobId>> for FBSearch {
    ...
    fn handle_new_best(&mut self, node: FBNode) -> FBNode {
        ...
        FBNode {
            forward_front: Vec::new(),
            backward_front: Vec::new(),
            forward_idle: Vec::new(),
            backward_idle: Vec::new(),
            remaining_processing_time: Vec::new(),
            nb_added: self.inst.nb_jobs() as usize,
            added: node.added,
            bound: local_state.v,
            idletime: 0 as i64,
            weightedidle: 0.,
...
impl FBSearch {
    pub fn new(filename: &str, objtype:Objective, guide:Guide, branchingscheme:BranchingScheme, use_ls:bool, solution_filename:Option<String>) -> Self {
        Self {
    ...
    pub fn add_job_forward(&self, node:&FBNode, j: JobId) -> FBNode {
        ...
        for m in 1..self.inst.nb_machines() {
            ...
            match self.objtype {
                Objective::Makespan => { res.remaining_processing_time[m as usize] -= self.inst.p(j,m); }
                _ => {}
            };
        ...
        match self.objtype {
            Objective::Makespan => { res.remaining_processing_time[0] -= self.inst.p(j,0); }
            _ => {}
        };
            ...
            Objective::Flowtime => {
                match self.branchingscheme {
                    BranchingScheme::Forward => {
                        ...
                        // let n = self.inst.nb_jobs()-node.nb_added as u32;  // nb remaining jobs
                        // res.bound += n as Time * (
                        //     res.forward_front.last().unwrap() - 
                        //     node.forward_front.last().unwrap()
                        // );
                    },
                    BranchingScheme::Bidirectional(_) => {
