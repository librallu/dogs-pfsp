...
fn generate_random_instance(nb_jobs:usize, nb_machines:usize) -> Rc<Instance> {
    let p_max:usize = 100;
    let v:Vec<Vec<Time>> = (0..nb_machines)
        .map(|_| {
            (0..nb_jobs).map(|_| fastrand::usize(..p_max) as Time).collect()
        })
        .collect();

    Rc::new(Instance::from_pmj(v))
}


fn main() {
    fastrand::seed(0);
    let yaml = load_yaml!("run_rand_inst.yml");
    let main_args = App::from_yaml(yaml).get_matches();
    let nb_jobs:usize = main_args.value_of("nb_jobs").unwrap().parse::<usize>().unwrap();
    let nb_machines:usize = main_args.value_of("nb_machines").unwrap().parse::<usize>().unwrap();
    let inst = generate_random_instance(nb_jobs, nb_machines);
    ...
    if let Some(branching_args) = main_args.subcommand_matches("f_flowtime") {
        ...
        run_search_tree(
            ...
            , t, perf_file.clone(), format!("IBS_forward_{:?}", guide)
        );
    }
    if let Some(branching_args) = main_args.subcommand_matches("neh_makespan") {
        ...
        run_search_tree(
            PruningDecorator::new(nehmakespan::NEHSearch::new(
                ...
                guide.clone(),
                sol_file.clone(),
            )), t, perf_file.clone(), format!("IBS_NEH_{:?}", guide)
        );
    }
